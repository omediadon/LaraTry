...
@section('page-content')
    <div class="container font-size-lg">
        @if($data['pagination']->currentPage() == 1)
            <x-media :all="$data['posts'][7]" class="mb-3 mb-lg-4 d-block d-lg-flex" :image="[
          'class' => 'align-self-center ml-lg-3 order-1 rounded img-fluid',
          'width' => [680]
        ]" :excerpt="['show' => true]" :body="[
        ...
        @foreach(array_chunk($data['pagination']->items(), 6) as $chunk)
            <div class="row">
                @foreach($chunk as $post)
                    <div class="col-md-4">
                        <x-card :all="$post" class="mb-3 mb-lg-4" :image="[
                'width' => [350],
                'height' => [233]
              ]" :body="[
                'headline' => [
        ...
        @if($data['pagination']->lastPage() > 1)
            <x-pagination class="justify-content-center mb-3 mb-lg-4" :current="$data['pagination']->currentPage()"
                          :total="$data['pagination']->total()" :next="[
