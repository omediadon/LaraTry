...
class Image extends Component{
	...
	public function __construct($all = [], $width = [], $height = [], $fit = '', $src = '', $alt = '', $id = '', $lazyload = true,
	                            $class = ''){
	...
	public function render(){
		...
		$this->attrs['data-srcset'] = $this->lazyloadSrc($this->getResizedImageSrc());
	...
	private function isExtern(){
		if(isset($this->getPathInfo()['dirname']) && strpos($this->getPathInfo()['dirname'], 'http') === false &&
		   strpos($this->getPathInfo()['dirname'], 'www') === false && strpos($this->getPathInfo()['dirname'], '//') === false){
			return false;
		}
		else{
			return true;
	...
	private function buildResizedImage(){
		...
		if(!File::exists($imageResizedPath)){
			...
			File::makeDirectory($this->getImageResizedFolder(), $mode = 0777, true, true);
			$image_resize->resize($sizes['width'], $sizes['height'], function($constraint){
	...
	private function getPlaceholder(){
		...
		if(File::exists($publicPlaceholderPath) === false){
			File::copy(resource_path('views/lbc/assets/images/placeholder.jpg'), $publicPlaceholderPath);
		}
	...
	private function isResize(){
		if(isset($this->width[0]) || isset($this->width[1]) || isset($this->height[0]) || isset($this->height[1])){
			return true;
		}
		else{
			return false;
