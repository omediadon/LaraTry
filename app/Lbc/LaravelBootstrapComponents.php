...
class LaravelBootstrapComponents{
	...
	public static function initDocs(){
		if(File::exists(public_path('lbc/docs/css/docs.min.css')) === false){
			File::copy(resource_path('views/lbc/css/docs.min.css'), public_path('lbc-docs.min.css'));
		}

		$domain = request()->getHost() === 'laravel-bootstrap-components.com' ? '' : '/lbc';
		Route::get($domain, [
			'uses' => '\App\Lbc\HomepageController@display',
			'as'   => 'homepage',
		]);
		Route::get($domain . '/components/{slug}', [
			'uses' => '\App\Lbc\ComponentsController@display',
			'as'   => 'components',
		])
		     ->where('slug', '(.*)');
		Route::get($domain . '/examples/{slug}', [
			'uses' => '\App\Lbc\ExamplesController@display',
			'as'   => 'examples',
		])
		     ->where('slug', '(.*)');
	...
	public static function initThemeLiara(){
		Route::get('/themes/liara', [
			'uses' => '\App\Lbc\Themes\Liara\HomepageController@home',
			'as'   => 'liara',
		]);
		Route::get('/themes/liara/features', [
			'uses' => '\App\Lbc\Themes\Liara\PagesController@features',
			'as'   => 'liara.features',
		]);
		Route::get('/themes/liara/imprint', [
			'uses' => '\App\Lbc\Themes\Liara\PagesController@imprint',
			'as'   => 'liara.imprint',
		]);
		Route::get('/themes/liara/about', [
			'uses' => '\App\Lbc\Themes\Liara\PagesController@about',
			'as'   => 'liara.about',
		]);
		Route::get('/themes/liara/blog', [
			'uses' => '\App\Lbc\Themes\Liara\BlogController@blog',
			'as'   => 'liara.blog',
		]);
		Route::get('/themes/liara/{slug}', '\App\Lbc\Themes\Liara\Helpers\RouteDispatch@get')
		     ->where('slug', '(.*)');
		Route::get('/themes/liara/category/{slug}', '\App\Lbc\Themes\Liara\Helpers\RouteDispatch@get')
		     ->where('slug', '(.*)');
		Route::get('/themes/liara/author/{slug}', '\App\Lbc\Themes\Liara\Helpers\RouteDispatch@get')
		     ->where('slug', '(.*)');
		Route::get('/themes/liara/result/', [
			'uses' => '\App\Lbc\Themes\Liara\BlogController@search',
			'as'   => 'liara.search',
		])
		     ->where('slug', '(.*)');
		Route::get('/themes/liara/contact/', [
			'uses' => '\App\Lbc\Themes\Liara\ContactController@contact',
			'as'   => 'liara.contact',
		]);
		Route::post('/themes/liara/contact/', [
			'uses' => '\App\Lbc\Themes\Liara\ContactController@send',
			'as'   => 'liara.contact',
		]);
		Route::post('/themes/liara/comment/send', [
			'uses' => '\App\Lbc\Themes\Liara\BlogController@send',
			'as'   => 'liara.send.comment',
		]);
	}
}
