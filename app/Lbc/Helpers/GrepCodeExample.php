...
class GrepCodeExample{
	static function get($exampleId){
		$file     = resource_path() . '/views/' . str_replace('lbc/components/', 'pages/components/', Request::path()) .
		            '.blade.php';
		$fh       = fopen($file, 'r');
		$continue = false;
		$i        = 0;
		...
		while($line = fgets($fh)){
			...
			if($continue && !strpos($line, 'example-start-' . $exampleId)){
				...
				if(strpos($line, '@php') && $i == 1){
					echo '<div class="tab-pane fade data" id="nav-data-' . $exampleId .
					     '" role="tabpanel"><pre class="prism-code mb-0 pb-3 pr-3 ">';
				}
				else if($i == 1){
					echo '<div class="tab-pane fade show active" id="nav-code-' . $exampleId .
					     '" role="tabpanel"><pre class="prism-code mb-0 pb-3 pr-3 ">';
				}
				...
				if(strpos($line, '@endphp')){
					echo '</pre></div>';
					echo '<div class="tab-pane fade code show active" id="nav-code-' . $exampleId .
					     '" role="tabpanel"><pre class="prism-code mb-0 pb-3 pr-3 ">';
				}
			}
		}
